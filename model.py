...
class SentimentRecommenderModel:
    ...
    ROOT_PATH = "pickle/"
    MODEL_NAME = "sentiment-classification-xg_hpt-boost-model.pkl"
    ...
    """Getting the top product 20 recommendations for the user"""

    def getRecommendationByUser(self, user):
        recommedations = []
        return list(self.user_final_rating.loc[user].sort_values(ascending=False)[0:20].index)

    """Filter the product recommendations using the sentiment model and get the top 5 recommendations using trained ML model"""
    ...
    """Classify the sentiment to 1/0 - positive or negative review by using the trained ML model"""
    ...
    """Preprocess the text before it is sent to ML model"""
    ...
    """Getting the pos tag to derive the lemma form"""
